...
class JarvisSavantEngine:
    ...
    def determine_bet_tier(
        ...
        """
        Determine bet tier based on FINAL score (v11.08 spec).

        FINAL = (research × 0.67) + (esoteric × 0.33) + confluence_boost

        Tiers (v11.08 spec):
        - TITANIUM_SMASH (2.5u): 3 of 4 engines >= 8.0 (special rule)
        ...
        Args:
            ...
            confluence: Confluence data with level and boost
            nhl_dog_protocol: Whether NHL dog protocol is triggered
            titanium_triggered: Whether Titanium rule is met (3/4 engines >= 8.0)
        """
        # Use tiering.py as single source of truth when available
        if TIERING_AVAILABLE:
            return tier_from_score(
        ...
        # Fallback: Legacy tier determination
        confluence_level = confluence.get("level", "DIVERGENT")

        # TITANIUM takes highest precedence (v11.08)
        if titanium_triggered:
            tier = "TITANIUM_SMASH"
            unit_size = 2.5
            explanation = "TITANIUM SMASH - Rare conviction. 3/4 engines aligned at 8.0+."
        # NHL Dog Protocol
        elif nhl_dog_protocol:
            tier = "ML_DOG_LOTTO"
            unit_size = 0.5

...
async def grader_status():
    """Check grader status - safe version with error handling."""
    result = {
        "available": True,
        "timestamp": datetime.now().isoformat()
    }
    ...
    try:
        if PICK_LOGGER_AVAILABLE:
            ...
            today_picks = pick_logger.get_picks_for_date(today)
            pending_picks = [p for p in today_picks if not getattr(p, 'result', None)]
            graded_picks = [p for p in today_picks if getattr(p, 'result', None)]
        ...
        result["pick_logger"] = {"available": False, "error": str(e)}
    ...
    try:
        ...
        if scheduler and hasattr(scheduler, 'auto_grade_job'):
            job = scheduler.auto_grade_job
            result["last_run_at"] = job.last_run.isoformat() if hasattr(job, 'last_run') and job.last_run else None
            result["last_errors"] = getattr(job, 'last_errors', [])[-5:]
        else:
    ...
    try:
        if AUTO_GRADER_AVAILABLE:
            ...
            result["weight_learning"] = {"available": False}
    except Exception as e:
        result["weight_learning"] = {"available": False, "error": str(e)}
